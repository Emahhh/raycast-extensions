...
import { isRecurring, displayDueDate, isExactTimeTask, displayDueDateTime } from "../helpers/dates";
...
export default function TaskListItem({ task, mode, viewProps, quickLinkView }: TaskListItemProps): JSX.Element {
  ...
  if (task.due?.date) {
    const isExactTime = isExactTimeTask(task);
    const text = isExactTime ? displayDueDateTime(task.due.date) : displayDueDate(task.due.date);
    ...
    if (mode === ViewMode.date && isExactTime) {
    ...
    if (mode !== ViewMode.date) {
      accessories.unshift({
        icon: recurring ? Icon.ArrowClockwise : Icon.Calendar,
  ...
  if (reminders.length > 0) {
    accessories.unshift({
      ...
      tooltip: `${reminders.length} reminder${reminders.length === 1 ? "" : "s"}: ${reminders
        .map((r) => displayDueDateTime(r.due.date))
        .join(", ")}`,
